...
export function getMessageType(message: Message): MessageType {
    const functions = [
        this.isActionMessage,
        this.isAnimatedMessage,
        this.isHeartMessage,
        this.isLiveVideoInviteMessage,
        this.isMediaMessage,
        this.isMediaShareMessage,
        this.isProfileShareMessage,
        this.isStoryShareMessage,
        this.isTextMessage,
        this.isVideoCallMessage,
        this.isVoiceMessage,
    ];
...
export function isActionMessage(message: Message): MessageType {
    return "action" in message ? MessageType.ACTION : null;
}

export function isAnimatedMessage(message: Message): MessageType {
    return "animated_media_images" in message ? MessageType.ANIMATED : null;
}

export function isHeartMessage(message: Message): MessageType {
    return "heart" in message ? MessageType.HEART : null;
}

export function isLiveVideoInviteMessage(message: Message): MessageType {
    return "live_video_invite" in message ? MessageType.LIVE_VIDEO_INVITE : null;
}

export function isMediaMessage(message: Message): MessageType {
    return "media" in message ? MessageType.MEDIA : null;
}

export function isMediaShareMessage(message: Message): MessageType {
...
export function isProfileShareMessage(message: Message): MessageType {
    return ["profile_share_name", "profile_share_username"].every((key) => key in message)
        ? MessageType.PROFILE_SHARE
        : null;
}

export function isStoryShareMessage(message: Message): MessageType {
    return "story_share" in message ? MessageType.STORY_SHARE : null;
}

export function isTextMessage(message: Message): MessageType {
    return "text" in message ? MessageType.TEXT_MESSAGE : null;
}

export function isVideoCallMessage(message: Message): MessageType {
    return "video_call_action" in message ? MessageType.VIDEO_CALL : null;
}

export function isVoiceMessage(message: Message): MessageType {
